...
def append_to_sheet(data, spreadsheet_id=None):
    ...
    try:
        ...
        # Limpeza do telefone (remove aspas, caracteres mudos e converte para Inteiro)
        # Converter para int garante que o Google Sheets entenda como NÚMERO e não TEXTO (sem aspa)
        raw_phone = str(data.get('whatsapp', ''))
        digits_only = ''.join(filter(str.isdigit, raw_phone))
        clean_phone = int(digits_only) if digits_only else ""
